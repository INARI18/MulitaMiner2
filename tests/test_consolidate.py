"""Consolidation identity and merge rules."""
from mulitaminer2.consolidate import consolidate_openvas, consolidate_tenable, normalize_name
...
def test_openvas_dedupe_merges_same_identity_keeps_most_complete():
    ...
    merged, log_lines = consolidate_openvas([a, b], allow_duplicates=False)
    assert len(merged) == 1
...
def test_openvas_allow_duplicates_keeps_everything():
    merged, _ = consolidate_openvas([_ov("X"), _ov("X")], allow_duplicates=True)
    assert len(merged) == 2
...
def test_cvss_zero_counts_as_filled():
    ...
    merged, _ = consolidate_openvas([with_zero, without], allow_duplicates=False)
...
def test_tenable_base_and_instances_blocks_pair_up_always():
    ...
    merged, _ = consolidate_tenable([base, inst], allow_duplicates=True)
    assert len(merged) == 1
...
def test_tenable_info_normalizes_to_log_after_pairing():
    ...
    rec.severity = "INFO"
    merged, _ = consolidate_tenable([rec], allow_duplicates=True)
